...
CONFIDENCE_TO_ACCEPT = .80

# load our serialized face detector from disk
print("[INFO] loading face detector...")
protoPath = os.path.sep.join([MODEL_PATH, MODEL_PROTOTXT])
modelPath = os.path.sep.join([MODEL_PATH, MODEL_PRE_TRAINEED])
...
# Seconds that must elapse to re-capture an image of non recognized face.
SECONDS_INTERVAL = 2
...
while True:
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > CONFIDENCE_TO_ACCEPT:
			...
			new_time = time.time()
			if (new_time - last_time) > SECONDS_INTERVAL:
				print(new_time - last_time)
				last_time = new_time
				img_item =  UNRECOGNIZED_FACES_PATH + str(len(os.listdir(UNRECOGNIZED_FACES_PATH))) + ".png"
				cv2.imwrite(img_item, frame_copy)
